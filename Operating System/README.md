...
공유된 데이터에 두 개 이상의 프로세스, 스레드가 동시에 접근시 데이터에 불일치가 일어날 수 있다.

__데이터의 일관성을 유지하기 위한 메커니즘__ 을 __동기화__ 라고 한다.
...
__임계 구역(critical section)__ 또는 공유변수 영역은 병렬컴퓨팅에서 둘 이상의 스레드가 동시에 접근해서는 안되는 공유 자원을 접근하는 코드의 일부를 말한다.
...
<img src="img/OS_DeadLock.JPG"></img><br/>

> 둘 이상의 프로세스가 각자 점유하고 있는 자원을 서로 기다릴 때 무한 대기에 빠지는 상황
...
DeadLock __발생__ 조건
 ...
 - 점유 대기
    - 자원을 하나 이상 보유하고 다른 프로세스의 자원을 점유하려 대기하는 프로세스가 존재해야 한다.
 - 비선점
...
데드락의 발생조건 4가지 중 하나라도 발생하지 않게 하는 것<br>
...
점유 대기 조건 방지 
- 프로세스 실행에 필요한 모든 자원을 한꺼번에 요청하고 획득하도록 한다.

- 프로세스가 자원을 갖고 있지 않을 때만 자원 요청 가능
  - 자원의 효율성이 낮다.
...
- 자원을 순환 형태로 대기하지 않도록 일정한 한 쪽 방향으로만 자원을 요구할 수 있도록 한다.
  - ex) 일련의 순서를 부여하고 한쪽 방향으로 자원 요청이 가능.<br> 어떤 프로세스가 R1을 사용했다면 다음에는 R2만 요청 가능
